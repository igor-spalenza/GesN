# ðŸ’° FLUXOGRAMA - PROCESSO DE VENDAS
...
## ðŸ”„ Fluxo Principal de CriaÃ§Ã£o de Venda/Pedido

```mermaid
flowchart TD
    %% === INÃCIO DO PROCESSO ===
    A[ðŸš€ UsuÃ¡rio inicia novo pedido] --> B[ðŸ“ Modal CriaÃ§Ã£o RÃ¡pida]
    
    %% === CRIAÃ‡ÃƒO RÃPIDA ===
    B --> C[ðŸ” SeleÃ§Ã£o de Customer<br/>via autocomplete]
    C --> D[ðŸ“… DefiniÃ§Ã£o de datas<br/>Pedido e Entrega]
    D --> E[ðŸšš Tipo de pedido<br/>Delivery/Pickup]
    E --> F[ðŸ’¾ Salvar OrderEntry<br/>Status: Pending]
    
    %% === ABERTURA EM ABA ===
    F --> G[ðŸ“‘ Abertura automÃ¡tica<br/>em nova aba dinÃ¢mica]
    
    %% === SELEÃ‡ÃƒO DE PRODUTO ===
    G --> H[ðŸ›ï¸ Buscar produto<br/>para adicionar]
    H --> I{ðŸŽ¯ Identificar<br/>ProductType}
    
    %% === FLUXOS POR TIPO DE PRODUTO ===
    
    %% PRODUTO SIMPLES
    I -->|Simple| J1[ðŸ“¦ Produto Simples]
    J1 --> J2[âš¡ Definir quantidade]
    J2 --> J3[ðŸ’° Aplicar preÃ§o padrÃ£o]
    J3 --> J4[âœ… Criar OrderItem<br/>imediatamente]
    J4 --> J5[ðŸ­ Gerar 1 Demand<br/>Status: Pending]
    
    %% PRODUTO COMPOSTO
    I -->|Composite| K1[ðŸ§© Produto Composto]
    K1 --> K2[ðŸŽ¨ Carregar hierarquias<br/>via CompositeProductXHierarchy]
    K2 --> K3[ðŸ“‹ Apresentar configuraÃ§Ã£o<br/>por camadas sequenciais]
    K3 --> K4{âš™ï¸ UsuÃ¡rio configura<br/>componentes}
    K4 --> K5[âœ”ï¸ Validar regras<br/>Min/Max, Opcionalidade]
    K5 --> K6[ðŸ’µ Calcular preÃ§o dinÃ¢mico<br/>Base + AdditionalCost]
    K6 --> K7[ðŸ“„ Salvar configuraÃ§Ã£o<br/>JSON em OrderItem]
    K7 --> K8[ðŸ­ Gerar 1 Demand +<br/>N ProductComposition]
    
    %% PRODUTO GRUPO
    I -->|Group| L1[ðŸ“¦ Grupo de Produtos]
    L1 --> L2[ðŸ“Š Carregar itens do grupo<br/>via ProductGroupItem]
    L2 --> L3[ðŸ”„ Verificar regras de troca<br/>ProductGroupExchangeRule]
    L3 --> L4{ðŸŽ›ï¸ Cliente configura<br/>quantidades e trocas}
    L4 --> L5[âš–ï¸ Validar proporÃ§Ãµes<br/>e limites de quantidade]
    L5 --> L6[ðŸ’° Calcular preÃ§o<br/>proporcional do kit]
    L6 --> L7[ðŸ“ Criar OrderItem principal]
    L7 --> L8[ðŸ­ Explodir grupo:<br/>Gerar N Demands<br/>uma por produto concreto]
    
    %% === CONSOLIDAÃ‡ÃƒO ===
    J5 --> M[ðŸ“Š Recalcular totais<br/>OrderEntry.TotalValue]
    K8 --> M
    L8 --> M
    
    M --> N{âž• Adicionar mais<br/>produtos?}
    N -->|Sim| H
    N -->|NÃ£o| O[ðŸ’³ Definir condiÃ§Ãµes<br/>de pagamento]
    
    %% === FINALIZAÃ‡ÃƒO ===
    O --> P[ðŸ‘€ RevisÃ£o final<br/>do pedido]
    P --> Q[âœ… Confirmar pedido<br/>OrderStatus: Confirmed]
    
    %% === INTEGRAÃ‡Ã•ES AUTOMÃTICAS ===
    Q --> R1[ðŸ­ PRODUÃ‡ÃƒO:<br/>Todas Demands â†’ Confirmed]
    Q --> R2[ðŸ’° FINANCEIRO:<br/>Gerar AccountReceivable]
    
    R1 --> S[ðŸŽ‰ Pedido finalizado<br/>e pronto para produÃ§Ã£o]
    R2 --> S
...
## ðŸ“‹ Detalhamento dos Fluxos por Tipo de Produto

### **ðŸ”· Fluxo: Produto Simples**
...
### **ðŸ”¶ Fluxo: Produto Composto**

#### **Processo de ConfiguraÃ§Ã£o Detalhado:**
```mermaid
flowchart TD
    A[ðŸ§© Produto Composto selecionado] --> B[ðŸ“Š Carregar CompositeProductXHierarchy]
    B --> C[ðŸŽ¯ Ordenar por AssemblyOrder]
    C --> D[ðŸ“‹ Para cada hierarquia]
    
    D --> E[ðŸ·ï¸ Exibir nome hierarquia]
    E --> F[ðŸ”§ Carregar ProductComponent<br/>da hierarquia]
    F --> G[âš™ï¸ Aplicar regras<br/>Min/Max/Optional]
    G --> H{ðŸŽ›ï¸ UsuÃ¡rio seleciona<br/>componentes}
    
    H --> I[âœ”ï¸ Validar quantidade<br/>dentro dos limites]
    I --> J[ðŸ’° Calcular custo adicional<br/>Î£ AdditionalCost]
    J --> K{ðŸ”„ PrÃ³xima hierarquia?}
    
    K -->|Sim| D
    K -->|NÃ£o| L[ðŸ’µ Calcular preÃ§o final<br/>Base + Adicionais]
    L --> M[ðŸ“„ Serializar configuraÃ§Ã£o<br/>para JSON]
    M --> N[ðŸ’¾ Salvar OrderItem]
    N --> O[ðŸ­ Gerar Demand +<br/>ProductComposition]
...
### **ðŸ”¸ Fluxo: Grupo de Produtos**

#### **Processo de ExplosÃ£o do Grupo:**
```mermaid
flowchart TD
    A[ðŸ“¦ Grupo selecionado] --> B[ðŸ“Š Carregar ProductGroupItem]
    B --> C[ðŸ”„ Verificar ProductGroupExchangeRule]
    C --> D[ðŸ“‹ Para cada item do grupo]
    
    D --> E{ðŸŽ¯ Item Ã© Product<br/>ou Category?}
    
    E -->|Product| F1[ðŸ“¦ Item especÃ­fico]
    F1 --> F2[âœ”ï¸ Validar produto ativo]
    F2 --> F3[ðŸ’¾ Adicionar Ã  lista<br/>de produtos concretos]
    
    E -->|Category| G1[ðŸ“‚ Categoria]
    G1 --> G2[ðŸ” Cliente escolhe<br/>produto da categoria]
    G2 --> G3[âœ”ï¸ Validar escolha]
    G3 --> F3
    
    F3 --> H{ðŸ”„ Aplicar regras<br/>de troca?}
    
    H -->|Sim| I1[âš–ï¸ Calcular proporÃ§Ãµes]
    I1 --> I2[ðŸ”„ Aplicar ExchangeRatio]
    I2 --> I3[âœ”ï¸ Validar limites finais]
    I3 --> J
    
    H -->|NÃ£o| J[ðŸ“ Gerar N OrderItem<br/>ou 1 OrderItem principal]
    J --> K[ðŸ­ Para cada produto concreto:<br/>Gerar Demand individual]
...
## ðŸ”„ Ciclo de Vida do Pedido (OrderStatus)
...
### **Alertas CrÃ­ticos:**
- ðŸš¨ **Demand sem ProductionOrder**: Demand confirmada > 2 horas sem agrupamento
- ðŸš¨ **Produto IndisponÃ­vel**: Tentativa de usar produto inativo
- ðŸš¨ **ConfiguraÃ§Ã£o InvÃ¡lida**: ProductComposition violando regras
- ðŸš¨ **Estoque Insuficiente**: Ingredientes necessÃ¡rios indisponÃ­veis
