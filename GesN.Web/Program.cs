...
string dbPath = Path.Combine(AppContext.BaseDirectory, "gesn.db");
...
builder.Services.AddScoped<IdentitySchemaInit>(provider =>
    new IdentitySchemaInit(connectionString));

builder.Services.AddScoped<DbInit>(provider =>
    new DbInit(connectionString));
